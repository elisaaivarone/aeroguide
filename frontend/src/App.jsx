  ...
  const searchFlight = async (searchNumber) => {
    ...
    try {
      const response = await fetch(
        `https://aeroguide-v57i.onrender.com/flights/${searchNumber}`,
      );
      ...
      setError(
        "Connection failed. Please check if the Python server is running and CORS is configured.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <main className="dashboard">
        ...
        {loading && (
          <div className="loading">
            📡 Fetching radar data and generating briefing...
          </div>
        )}
        ...
        {result && result.details && (
          <div className="result-card">
            <div className="airline-banner">
              <img
                src="https://images.unsplash.com/photo-1436491865332-7a61a109cc05?auto=format&fit=crop&w=800&h=200&q=80"
                alt="Avião Comercial"
                className="airplane-photo"
              />
              <div className="airline-info">
                <img
                  src={`https://pics.avs.io/200/200/${result.details.airline?.iata || flight.substring(0, 2).toUpperCase()}.png`}
                  alt="Logo da Companhia"
                  className="airline-logo"
                  onError={(e) => (e.target.style.display = "none")}
                />
                <span className="airline-name">
                  {result.details.airline?.name || "Companhia Aérea"}
                </span>
              </div>
            </div>
            ...
            <div className="flight-header-box">
              ...
              <div className="flight-route">
                <span className="iata">
                  {result.details.departure?.iata?.toUpperCase() || "N/A"}
                </span>
                <span className="arrow">→</span>
                <span className="iata">
                  {result.details.arrival?.iata?.toUpperCase() || "N/A"}
                </span>
              </div>
              <div className="flight-status">
                {result.details.flight_status
                  ? result.details.flight_status.toUpperCase()
                  : "UNKNOWN"}{" "}
                (Operational)
              </div>
            </div>
                ...
                <div className="ops-times-grid">
                  <div className="time-item">
                    ...
                    <span className="time-value">
                      {formatTime(result.details.departure?.scheduled)}
                    </span>
                  </div>
                  <div className="time-item">
                    <span className="time-label">ESTIMATED</span>
                    <span className="time-value highlighted">
                      {formatTime(result.details.departure?.estimated)}
                    </span>
                  </div>
                </div>
                ...
                <div className="ops-gates-grid">
                  <div className="gate-item">
                    ...
                    <span className="gate-value">
                      {result.details.departure?.terminal || "S/N"}
                    </span>
                  </div>
                  <div className="gate-item">
                    <span className="gate-label">GATE</span>
                    <span className="gate-value">
                      {result.details.departure?.gate || "S/N"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="ops-box arrival-box">
                ...
                <div className="ops-times-grid">
                  <div className="time-item">
                    <span className="time-label">SCHEDULED</span>
                    <span className="time-value">
                      {formatTime(result.details.arrival?.scheduled)}
                    </span>
                  </div>
                  <div className="time-item">
                    <span className="time-label">ESTIMATED</span>
                    <span className="time-value highlighted">
                      {formatTime(result.details.arrival?.estimated)}
                    </span>
                  </div>
                </div>
                ...
                <div className="ops-gates-grid">
                  <div className="gate-item">
                    <span className="gate-label">TERMINAL</span>
                    <span className="gate-value">
                      {result.details.arrival?.terminal || "S/N"}
                    </span>
                  </div>
                  <div className="gate-item">
                    <span className="gate-label">GATE</span>
                    <span className="gate-value">
                      {result.details.arrival?.gate || "S/N"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Mapa operacional */}
            {result.details.departure?.iata &&
              result.details.arrival?.iata &&
                ...
                <div className="map-container-ops">
                  ...
                  >
                    ...
                    <Marker
                      position={
                        airportCoords[
                          result.details.departure.iata.toUpperCase()
                        ]
                      }
                    >
                      <Popup>
                        Origin: {result.details.departure.iata.toUpperCase()}
                      </Popup>
                    </Marker>
                    <Marker
                      position={
                    ...
                    >
                      <Popup>
                        Destination: {result.details.arrival.iata.toUpperCase()}
                      </Popup>
                    </Marker>
                    <Polyline
                      positions={[
                        ...
                        airportCoords[
                          result.details.arrival.iata.toUpperCase()
                        ],
                      ]}
                      color="orange"
                      weight={4}
